...
export default function RequestDetailPage() {
  ...
  const fetchAddOnsDetails = async (selectedAddOns, serviceType) => {
    try {
      ...
      if (addOnsArray.length > 0) {
        const selectedAddOnsWithDetails = Object.entries(selectedAddOns)
          .map(([addOnId, config]) => {
  ...
  const canPay = ["ACCEPTED", "AWAITING_PAYMENT"].includes(
    ...
    <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-[1600px] mx-auto">
      ...
      <div className="bg-gradient-to-br from-zinc-800/60 to-zinc-900/60 border border-zinc-700/50 rounded-xl p-8 mb-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          />
          <InfoCard icon={Music} label="Tier" value={currentRequest.tier} />
          <InfoCard
            icon={Calendar}
            label="Created"
          ...
          <Section title="Financial Details">
            <div className="space-y-3">
              <div className="flex justify-between text-gray-300">
                <span>
                  {currentRequest.tier} Tier ({currentRequest.services})
                </span>
