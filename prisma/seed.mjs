...
async function main() {
  ...
  if (defaultTiers.length > 0) {
    console.log(
      "🌱 Creating universal tiers with service-specific descriptions...",
    );
    for (const tierData of defaultTiers) {
      try {
        ...
        console.log(
          `✅ Created tier: ${tier.name} (order: ${tier.order}) - $${tier.price}`,
        );
      } catch (error) {
        console.error(
          `❌ Error creating tier ${tierData.name}:`,
          error.message,
        );
      }
    }
    console.log(
      `🎉 All ${defaultTiers.length} universal tiers created successfully!`,
    );
  } else {
  ...
  if (defaultAddOns.length > 0) {
    ...
    for (const addOnData of defaultAddOns) {
      try {
        ...
        console.log(
          `✅ Created/Updated add-on: ${addOnData.serviceType} - ${addOnData.name}`,
        );
      } catch (error) {
        console.error(
          `❌ Error creating add-on ${addOnData.name}:`,
          error.message,
        );
      }
    }
    console.log(
      `🎉 All ${defaultAddOns.length} add-ons processed successfully!`,
    );
  } else {
  ...
  if (defaultConditions.length > 0) {
    ...
    for (const conditionData of defaultConditions) {
      try {
        ...
        console.log(
          `✅ Created/Updated condition: ${conditionData.serviceType} - ${conditionData.fieldName}`,
        );
      } catch (error) {
        console.error(
          `❌ Error creating condition ${conditionData.fieldName}:`,
          error.message,
        );
      }
    }
    console.log(
      `🎉 All ${defaultConditions.length} acceptance conditions processed successfully!`,
    );
  } else {
  ...
  if (defaultProviders.length > 0) {
    ...
    for (const providerData of defaultProviders) {
      try {
        ...
        console.log(
          `✅ Created/Updated payment provider: ${providerData.name}`,
        );
      } catch (error) {
        console.error(
          `❌ Error creating payment provider ${providerData.name}:`,
          error.message,
        );
      }
    }
    console.log(
      `🎉 All ${defaultProviders.length} payment providers processed successfully!`,
    );
  } else {
